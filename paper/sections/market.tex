...
platform, in order to understand the degree of elasticity of the supply. If the
...
amount of ``work done" gets split among a higher number of tasks. To
...
1000's)" indicates an intercept of 2.5 and a slope of 0.05\footnote{We use Ordinary Least Squares regression.}. To put
...
On the demand side, we observe some requesters frequently posting new batches of recurrent tasks. Hence, we are interested in the periodicity of such demand in the marketplace and the supply it drives. To look in this, we consider both the time-series of available HITs and the rewards completed over a period of three months. 
...
Conversely, and to check for the periodicity in the supply, we compute an autocorrelation on the weekly moving average convergence of the reward attached to completed HITs. Figure \ref{fig:mac} and \ref{fig:acmac} show that there is a strong weekly periodicity effect, as we observe high values in the range 0-250 hours.
